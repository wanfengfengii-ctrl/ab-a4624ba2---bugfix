...
- 约束二：任意**不同**载波 i、j 产生的三阶互调产物 `2fi − fj`，与任意
  ...
  及产物与第三载波的**实际距离**。
- 频点必须是**安全整数**（`Number.isSafeInteger`，即不超过 `2^53−1` 的正整数
  kHz）。互调产物 `2fi − fj` 可能略超出该范围（例如高频合法频点
  `fi=2^53−3、fj=2^53−7` 的产物为 `2^53+1`），此时 number 浮点运算会舍入
  失真；所有互调产物与距离均按 **BigInt 精确整数**核算，因此“距离恰好
  等于保护间隔”始终判定为合法，不会被浮点舍入误判为碰撞。
